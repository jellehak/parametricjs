...
function enqueue (fn) {
  Promise.resolve().then(() => {
    try { fn() } catch (e) { hostReportError(e) }
  })
}
...
export class Observable {
  ...
  map (fn) {
    ...
    return new C(observer => this.subscribe({
      next (value) {
        try { value = fn(value) } catch (e) { return observer.error(e) }
        observer.next(value)
      },
  ...
  filter (fn) {
    ...
    return new C(observer => this.subscribe({
      next (value) {
        try { if (!fn(value)) return } catch (e) { return observer.error(e) }
        observer.next(value)
      },
      error (e) { observer.error(e) },
      complete () { observer.complete() }
  ...
  reduce (fn) {
    ...
    return new C(observer => this.subscribe({
      ...
      next (value) {
        ...
        if (!first || hasSeed) {
          try { acc = fn(acc, value) } catch (e) { return observer.error(e) }
        } else {
          acc = value
        }
      },
  ...
  flatMap (fn) {
    ...
    return new C(observer => {
      ...
      let outer = this.subscribe({
        next (value) {
          if (fn) {
            try { value = fn(value) } catch (e) { return observer.error(x) }
          }
