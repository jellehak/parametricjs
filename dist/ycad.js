...
var _csg = __webpack_require__(4);

var _csg2 = _interopRequireDefault(_csg);

var _ThreeCSG = __webpack_require__(5);

var _ThreeCSG2 = _interopRequireDefault(_ThreeCSG);

var _insert = __webpack_require__(6);
...
var _PathToShape = __webpack_require__(8);

var _PathToShape2 = _interopRequireDefault(_PathToShape);
...
function YCAD(_cad) {
    ...
    this.getPart = function (id) {
        return _lodash2.default.find(cadData.parts, { id: id });
    };

    this.getParameters = function () {
        return cadData.parameters || {};
    };
    ...
    this.getFeatureLabels = function () {
        var ret = [];
        for (var k in cadData.features) {
            ret.push({ label: cadData.features[k].type });
        }
        return ret;
    };
    ...
    this.getParametersDefault = function () {
        var ret = {};
        for (var k in cadData.parameters) {
    ...
    function render() {
        ...
        for (var k in cadData.features) {
            var feature = cadData.features[k];
    ...
    function compileParameters() {
        ...
        var _compiled = "";
        for (var key in cadData.parameters) {
            //let value = cadData.parameters[key]
            var value = getParameter(cadData.parameters[key]).default;
            _compiled += "let " + key + " = " + value + ";";
        }
        //console.log(cadData.parameters, _compiled)
        //save compile to feature
        cadData._parameters = _compiled; // || "height=10;width=3;thickness=1";
    }
    ...
    function getParameter(parameter) {
        if ((typeof parameter === "undefined" ? "undefined" : _typeof(parameter)) === 'object') {
            return parameter;
        }
    ...
    function compile(what) {
        var _eval = cadData._parameters + "; \n " + what;

        if (debug) console.log("Compiling : " + _eval);
        return eval(_eval);
    }
    ...
    function processFeature(feature) {
        ...
        switch (feature.type) {
            ...
            case 'sketch':
                ...
                for (var k2 in path) {
                    ...
                    var _point = { x: point.x, y: point.y
                        ...
                        //Process macro
                    };if ((0, _isString2.default)(point.x)) {
                        _point.x = compile(point.x);
                    }
                    if ((0, _isString2.default)(point.y)) {
            ...
            case 'line':
                //Add points to scene
                var p1 = feature.data.path[0];
                var p2 = feature.data.path[1];
                y3d.line(p1, p2);
                break;

            case 'insert':
                (0, _insert2.default)(input);
                break;
            ...
            case 'extrude':
                ...
                var extrudeSettings = data.settings;
                var _extrudeSettings = Object.create(data._settings);

                //Compile
                _extrudeSettings.amount = compile(extrudeSettings.amount);
                //console.log(extrudeSettings.amount, _extrudeSettings.amount)

                var what = getFeatureById(data.selectById || data.sketchId);
                ...
                var shape = (0, _PathToShape2.default)(what.data._path);
            ...
            default:
                if (debug) console.log("Unknown feature: ", feature.type);
        }
    }

    function getFeatureById(id) {
        return _lodash2.default.find(cadData.features, { 'id': id });
    }
...
window.ThreeBSP = function () {
	...
	_ThreeBSP = function ThreeBSP(geometry) {
		...
		} else if (geometry instanceof _ThreeBSP.Node) {
			this.tree = geometry;
			this.matrix = new THREE.Matrix4();
			return this;
		} else {
		...
		for (i = 0, _length_i = geometry.faces.length; i < _length_i; i++) {
			...
			polygon = new _ThreeBSP.Polygon();

			if (face instanceof THREE.Face3) {
				vertex = geometry.vertices[face.a];
				uvs = faceVertexUvs ? new THREE.Vector2(faceVertexUvs[0].x, faceVertexUvs[0].y) : null;
				vertex = new _ThreeBSP.Vertex(vertex.x, vertex.y, vertex.z, face.vertexNormals[0], uvs);
				vertex.applyMatrix4(this.matrix);
				polygon.vertices.push(vertex);

				vertex = geometry.vertices[face.b];
				uvs = faceVertexUvs ? new THREE.Vector2(faceVertexUvs[1].x, faceVertexUvs[1].y) : null;
				vertex = new _ThreeBSP.Vertex(vertex.x, vertex.y, vertex.z, face.vertexNormals[1], uvs);
				vertex.applyMatrix4(this.matrix);
				polygon.vertices.push(vertex);

				vertex = geometry.vertices[face.c];
				uvs = faceVertexUvs ? new THREE.Vector2(faceVertexUvs[2].x, faceVertexUvs[2].y) : null;
				vertex = new _ThreeBSP.Vertex(vertex.x, vertex.y, vertex.z, face.vertexNormals[2], uvs);
				vertex.applyMatrix4(this.matrix);
				polygon.vertices.push(vertex);
				...
				vertex = new _ThreeBSP.Vertex(vertex.x, vertex.y, vertex.z, face.vertexNormals[3], uvs);
				vertex.applyMatrix4(this.matrix);
				polygon.vertices.push(vertex);
			} else {
		...
		this.tree = new _ThreeBSP.Node(polygons);
	};
	...
	_ThreeBSP.Polygon = function (vertices, normal, w) {
	...
	_ThreeBSP.Polygon.prototype.calculateProperties = function () {
	...
	_ThreeBSP.Polygon.prototype.clone = function () {
		var i,
		    vertice_count,
		    polygon = new _ThreeBSP.Polygon();
	...
	_ThreeBSP.Polygon.prototype.flip = function () {
		var i,
		    vertices = [];
	...
	_ThreeBSP.Polygon.prototype.classifyVertex = function (vertex) {
	...
	_ThreeBSP.Polygon.prototype.classifySide = function (polygon) {
		var i,
		    vertex,
		    classification,
		    num_positive = 0,
	...
	_ThreeBSP.Polygon.prototype.splitPolygon = function (polygon, coplanar_front, coplanar_back, front, back) {
		...
		if (classification === COPLANAR) {
			...
			if (f.length >= 3) front.push(new _ThreeBSP.Polygon(f).calculateProperties());
			if (b.length >= 3) back.push(new _ThreeBSP.Polygon(b).calculateProperties());
		}
	};

	_ThreeBSP.Vertex = function (x, y, z, normal, uv) {
		this.x = x;
		this.y = y;
		this.z = z;
	...
	_ThreeBSP.Vertex.prototype.clone = function () {
		return new _ThreeBSP.Vertex(this.x, this.y, this.z, this.normal.clone(), this.uv.clone());
	};
	_ThreeBSP.Vertex.prototype.add = function (vertex) {
		this.x += vertex.x;
		this.y += vertex.y;
		this.z += vertex.z;
		return this;
	};
	_ThreeBSP.Vertex.prototype.subtract = function (vertex) {
		this.x -= vertex.x;
		this.y -= vertex.y;
		this.z -= vertex.z;
		return this;
	};
	_ThreeBSP.Vertex.prototype.multiplyScalar = function (scalar) {
		this.x *= scalar;
		this.y *= scalar;
		this.z *= scalar;
		return this;
	};
	_ThreeBSP.Vertex.prototype.cross = function (vertex) {
		var x = this.x,
		    y = this.y,
		    z = this.z;
	...
	_ThreeBSP.Vertex.prototype.normalize = function () {
	...
	_ThreeBSP.Vertex.prototype.dot = function (vertex) {
		return this.x * vertex.x + this.y * vertex.y + this.z * vertex.z;
	};
	_ThreeBSP.Vertex.prototype.lerp = function (a, t) {
	...
	_ThreeBSP.Vertex.prototype.interpolate = function (other, t) {
		return this.clone().lerp(other, t);
	};
	_ThreeBSP.Vertex.prototype.applyMatrix4 = function (m) {
	...
	_ThreeBSP.Node = function (polygons) {
		var i,
		    polygon_count,
		    front = [],
		    back = [];
		...
		if (front.length > 0) {
			this.front = new _ThreeBSP.Node(front);
		}

		if (back.length > 0) {
			this.back = new _ThreeBSP.Node(back);
		}
	};
	_ThreeBSP.Node.isConvex = function (polygons) {
		var i, j;
	...
	_ThreeBSP.Node.prototype.build = function (polygons) {
		var i,
		    polygon_count,
		    front = [],
		    back = [];
		...
		if (front.length > 0) {
			if (!this.front) this.front = new _ThreeBSP.Node();
			this.front.build(front);
		}

		if (back.length > 0) {
			if (!this.back) this.back = new _ThreeBSP.Node();
			this.back.build(back);
		}
	};
	_ThreeBSP.Node.prototype.allPolygons = function () {
		var polygons = this.polygons.slice();
		if (this.front) polygons = polygons.concat(this.front.allPolygons());
		if (this.back) polygons = polygons.concat(this.back.allPolygons());
		return polygons;
	};
	_ThreeBSP.Node.prototype.clone = function () {
		var node = new _ThreeBSP.Node();
	...
	_ThreeBSP.Node.prototype.invert = function () {
	...
	_ThreeBSP.Node.prototype.clipPolygons = function (polygons) {
	...
	_ThreeBSP.Node.prototype.clipTo = function (node) {
...
function insert(input) {
    ...
    var partData = ycad.getPart(feature.data.selectInnerPart);
    console.log(partData);
    //part.

    var part = new _index2.default(partData);
...
exports.default = function (input) {
    ...
    var plane = new THREE.Mesh(new THREE.PlaneGeometry(10, 10), new THREE.MeshPhongMaterial({ color: 0x0000ff, transparent: true, opacity: 0.2 }));
    plane.doubleSided = true;
    //plane.position.x = 100;
    //plane.rotation.z = 2;  // Not sure what this number represents.
    scene.add(plane);
